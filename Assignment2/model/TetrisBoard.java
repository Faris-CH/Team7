...
public class TetrisBoard implements Serializable{
    ...
    public int placementHeight(TetrisPiece piece, int x) {
        ...
        // Subtracts the lowest Y val from the col height and sees if it is bigger than
        // the current resting Y val
        for (int i = 0; i < piece.getWidth(); i++) {

            if (this.getColumnHeight(x + i) - piece.getLowestYVals()[i] > rest){
                rest = this.getColumnHeight(x + i) - piece.getLowestYVals()[i];
            }
        }
        return rest;
    }
    ...
    public int placePiece(TetrisPiece piece, int x, int y) {
        ...
        for (TetrisPoint p:
            ...
            // Checks if the piece is out of the board
            if (p.x + x >= width || p.x + x < 0 || p.y + y >= height || p.y + y < 0) {
                return ADD_OUT_BOUNDS;
            }


            // Checks if the piece is colliding with another piece
            if(tetrisGrid[p.x + x][p.y + y] == true){
                return ADD_BAD;
            }
        }

        if (y == placementHeight(piece, x)){
            committed = false;
            for (TetrisPoint p:
                    piece.getBody()) {
                this.tetrisGrid[p.x + x][p.y + y] = true;
                if (colCounts[p.x + x] <= y + p.y){
                    colCounts[p.x + x] = y + p.y + 1;
                }
                rowCounts[p.y + y] += 1;

                if (rowCounts[p.y + y] == width){
                    filled = true;
                }
            }
        } else {
            ...
            for (TetrisPoint p:
                ...
                this.tetrisGrid[p.x + x][p.y + y] = true;
                if (colCounts[p.x + x] <= y + p.y){
                    colCounts[p.x + x] = y + piece.getHeight();
                }
            }
        }
    ...
    public int clearRows() {
        ...
        for(int i = 0; i < maxH; i++) {
            ...
            // checks if the row is clearable and deletes the row
            ...
            // If the row is not clearable, shifts it down if the previous rows have been cleared and deletes the row
            else if (rowsCleared > 0){
                for(int k = 0; k < getWidth(); k++){
                    tetrisGrid[k][i - rowsCleared] = tetrisGrid[k][i];
                    tetrisGrid[k][i] = false;
                }
            }
        }
