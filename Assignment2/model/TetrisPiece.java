...
public class TetrisPiece implements Serializable {
    ...
    */
    ...
    private TetrisPiece prev; // We'll use this to link each piece to its "previous" rotation.
    ...
    public boolean equals(Object obj) {
        ...
        boolean equals = true;
        if ((((TetrisPiece) obj).getBody().length == this.body.length) == false){
            return false;
        }
        for (TetrisPoint p:
                ((TetrisPiece) obj).getBody()) {
            if(Arrays.stream(this.body).anyMatch(Predicate.isEqual(p)) == false){
                equals = false;
            }
        }
        return equals;
    ...
    /**
     * Returns a pre-computed piece that is 90 degrees clockwise
     * rotated from the receiver. Fast because the piece is pre-computed.
     * This only works on pieces set up by makeFastRotations(), and otherwise
     * just returns null.
     *
     * @return the next rotation of the given piece
     */
    public TetrisPiece fastAntiRotation() {
        return prev;
    }
    ...
    public static TetrisPiece makeFastRotations(TetrisPiece root) {
        TetrisPiece new_root = root.computeNextRotation();

        root.next = makeFastRotationsHelper(root, new_root, root);

        return root;
    }

    private static TetrisPiece makeFastRotationsHelper(TetrisPiece mainRoot, TetrisPiece root, TetrisPiece prevRoot){
        if (root.computeNextRotation().equals(mainRoot)){
            root.next = mainRoot;
            root.prev = prevRoot;
            mainRoot.prev = root;
            return root;
        ...
        root.prev = prevRoot;
        root.next = makeFastRotationsHelper(mainRoot, root.computeNextRotation(), root);
    ...
    public TetrisPiece computeNextRotation() {
        int lowest_y = 0;
        TetrisPoint[] new_points = new TetrisPoint[this.body.length];
        
        // Goes through every point and changes the x-value to y-value, and the y-value to -(x-value)
        // Determines the lowest y-value
        for (int i = 0; i < body.length; i ++) {
            TetrisPoint point = new TetrisPoint(0, 0);
            point.x = body[i].y;
            point.y = -body[i].x;
        ...
        // Changes the origin of the piece to (0, 0) by making all y-values positive
        if (lowest_y < 0){
            for (TetrisPoint p:
                 new_points) {
                p.y -= lowest_y;
        ...
        return new TetrisPiece(new_points);
    }
    /**
